    ...
    For X = 1 To 100
        If InMapBounds(X, Y) Then
            ...
            If SobreX = X And SobreY = Y Then
                ' Pone Grh !
                Sobre = -1
                If frmMain.cSeleccionarSuperficie.value = True Then 'si esta el boton de "insertar superficie" apretado
            ...
            End If
            'Layer 2 **********************************
            If MapData(X, Y).Graphic(2).GrhIndex <> 0 And (VerCapa2 = True) Then
    ...
    For X = 1 To 100
        If InMapBounds(X, Y) Then
            ...
            iPPy = ((32 * ScreenY) - 32) + 0
             'Object Layer **********************************
             If MapData(X, Y).OBJInfo.objindex <> 0 And VerObjetos = True Then
                 Call DDrawTransGrhtoSurface(BMPSurface, MapData(X, Y).ObjGrh, iPPx, iPPy, 1, 1)
             End If
            
            'Char layer **********************************
            If MapData(X, Y).CharIndex <> 0 And VerNpcs = True Then
            ...
            'Layer 3 *****************************************
            If MapData(X, Y).Graphic(3).GrhIndex <> 0 And (VerCapa3 = True) Then
...
For Y = 1 To 100
    ...
    For X = 1 To 100
        If X < 101 And X > 0 And Y < 101 And Y > 0 Then
            If MapData(X, Y).Graphic(4).GrhIndex <> 0 And VerCapa4 = True Then
                Call DDrawTransGrhtoSurface(BMPSurface, MapData(X, Y).Graphic(4), ((32 * ScreenX) - 32) + 0, ((32 * ScreenY) - 32) + 0, 1, 1)
            End If
            
            If MapData(X, Y).TileExit.Map <> 0 And VerTranslados = True Then
                Grh.GrhIndex = 3
                Grh.FrameCounter = 1
                Grh.Started = 0
            ...
            'Show blocked tiles
            If VerBloqueos And MapData(X, Y).Blocked = 1 Then
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''
If RenderToBMP Then
    SavePicture frmRenderer.Picture1.Picture, App.Path & "\Renders\" & NombreImagen & ".bmp"
Else
    Set C = New cDIBSection
    C.CreateFromPicture frmRenderer.Picture1.Picture
    Call SaveJPG(C, App.Path & "\Renders\" & NombreImagen & ".jpg")
    Set C = Nothing
End If
''''''''''''''''''''''''''''''''''''''''''''''''''''''''
